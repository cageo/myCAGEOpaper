%
% This function performs the outer loop of the PPM procedure for HGS
% Author : Thomas Hermans
% Date : November 2012
% Modified after the ppm code of Stanford
% Author: Celine Scheidt
% Date: May 2011


function [outer_opt inner_opt ofmin nbsimu] = PPM_Sgems_HGS(obj,nitr,x0,OF_x0,WorkingDirectory,obs_response,rand_seed,...
...
% T.H. ofmin = minimum value of objective function
% T.H. nbsimu = number of simulations
...
if nargin < 16  % If regions are not provided --> Global PPM
    rd_regions = ones(snesim_input.nx*snesim_input.ny*snesim_input.nz,1);
end
% T.H. Number of regions
npara = length(unique(rd_regions));
% T.H. Don't understand why the next lines are done
rand('state',rand_seed);
rand('seed',rand_seed);
% T.H. Generate nitr random seeds for the outer loop
rand_vals = round(rand(1,nitr)*1000000);  % definition of random seeds
% T.H. Verify the objective function to see if the model is history matched
if OF_x0 < obj
    fprintf('Initial realization is already History Matched \n')
    return
end
...
if npara > 1
   ...
   elseif size(obs_response,2) > 1
       error('Local PPM cannot be performed using distance.  Please give as input the data to be matched and not the responses for the initial models');
   elseif size(obs_response,1) ~= npara
       error('The number of regions in the PPM is different than the number of wells.');
   end
end
   
% parameters for brent optimization
% T.H. This is the method chosen for optimization
ax = 0; % minimal value for rD
% T.H. repmat replicate the matrix 0.5 as many time as there are regions
% not necessary tu use repmat for a singe region
bx = repmat(0.5,1,npara); % starting value for rD
cx = repmat(1,1,npara);   % maximal value for rD
% T.H. OF_x0 is a vector whose length is the number of wells
...
for iter = 1:nitr  % outer loop
  ...
  % T.H. This a function_handle in order to pass a function through other
  % function, this will enable to recompute the OF for different rD using
  % this handle and use it in other functions
   head_objfun = @(rD) ComputeOFgivenRD_HGS(rD,x0,[WorkingDirectory directory_results],obs_response,rand_vals(iter),...
                       PriorProba,snesim_input,Prj_name,SimulationDeck,SimulationProp,SimulationArraySize,PropResponse,SoftProba_file,rd_regions);
